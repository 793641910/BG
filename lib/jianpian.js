var rule = {
    ...
    is_json:1,
    tabs:'js:TABS=[];if(html.data.have_ftp_ur == 1){TABS.push("兵哥视界超清版")}if(html.data.have_m3u8_ur == 1){TABS.push("兵哥视界普清版")}',
    lists:'js:log(TABS);LISTS=[];TABS.forEach(function(tab){if(/兵哥视界/.test(tab)){let ftp=html.data.new_ftp_list;let d=ftp.map(function(it){return it.title+"$"+(/m3u8/.test(it.url)?play_url+it.url:"tvbox-xg:"+it.url)});LISTS.push(d)}else if(/兵哥视界/.test(tab)){let m3u=html.data.new_m3u8_list;let d=m3u.map(function(it){return it.title+"$"+(/m3u8/.test(it.url)?play_url+it.url:"tvbox-xg:"+it.url)});LISTS.push(d)}});',
    },
